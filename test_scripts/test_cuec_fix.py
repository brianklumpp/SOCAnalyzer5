...
# Set up job paths - the extractor will look for output.txt and section_results.json here
job_paths = {
...
if cuec_result_path.exists():
    ...
    print(f"\nChecking first 3 CUECs for line_ref and page_refs:")
    for i, cuec in enumerate(cuecs[:3]):
        line_ref = cuec.get('cuec_line_ref')
        page_refs = cuec.get('cuec_page_refs', [])
        desc = cuec.get('cuec_description', '')[:60]
        print(f"\n  CUEC {i+1}:")

# control_extractor_combined.py

"""
Combined SOC 1 + SOC 2 Control Extractor - Dual Framework Mapping
==================================================================
...
4. DUAL FRAMEWORK MAPPING - Attempt both TSC and financial assertion mapping
5. WEIGHTED CONFIDENCE - Calculate framework confidence (60% primary + 40% match density)
6. AMBIGUITY FLAGGING - Mark controls with low confidence (<70%) for manual review

Combined Report Logic:
- Attempts TSC mapping first (SOC 2 methodology)
- Attempts financial assertion mapping second (SOC 1 methodology)
- Calculates weighted score: primary_confidence * 0.6 + match_density * 0.4
- If TSC confidence > Financial confidence: framework_category = "SOC2"
- If Financial confidence > TSC confidence: framework_category = "SOC1"
- If max_confidence < 70%: framework_category = "AMBIGUOUS" (manual review needed)
- Stores both mappings when confidence > 50% for either framework
"""
...
try:
    ...
    print(f"[CONTROL_EXTRACTOR_COMBINED] Import error: {import_err}")
    raise

# Configure logging
log_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'data', 'logs', 'control_extractor_combined.log')
logging.basicConfig(
    filename=log_path,
    filemode='w',
    level=logging.INFO,
    format='%(asctime)s [CONTROL_EXTRACTOR_COMBINED] %(message)s',
)

# ============================================================================
# COMBINED PROMPT - Uses SOC 1 prompt as base with TSC mapping addition
# ============================================================================
# Will use CONTROL_EXTRACTION_PROMPT_V4_SOC1 and add TSC logic in post-processing
...
# ============================================================================
# DUAL FRAMEWORK MAPPING - TSC + Financial Assertions
# ============================================================================

def map_tsc_criteria(control: Dict[str, Any]) -> Dict[str, Any]:
    """
    Attempt TSC mapping for combined reports (SOC 2 methodology).
    
    Logic:
    1. Search control text for TSC patterns (CC, A, C, PI, P)
    2. Calculate confidence based on explicit mentions
    3. Return TSC mappings with confidence scores
    
    Returns:
        Dict with tsc_mappings, tsc_confidence, tsc_match_density
    """
    control_text = control.get('control_description', '') + ' ' + control.get('control_objective', '')
    control_text = control_text.lower()
    
    # TSC pattern: CC1.1, A1.2, C1.1, PI1.1, P1.1
    TSC_PATTERN = re.compile(r'\b(CC|A|C|PI|P)[-.]?(\d+)\.(\d+)\b', re.IGNORECASE)
    
    tsc_matches = TSC_PATTERN.findall(control_text)
    tsc_mappings = []
    
    for category, point, subpoint in tsc_matches:
        tsc_code = f"{category.upper()}{point}.{subpoint}"
        tsc_mappings.append({
            "tsc_code": tsc_code,
            "confidence": 0.95,  # Explicit mention = high confidence
            "reasoning": f"Explicit mention in control text"
        })
    
    # Calculate match density
    tsc_match_density = len(tsc_matches) / max(len(control_text.split()), 1)
    
    # Confidence: high if explicit mentions, low otherwise
    tsc_confidence = 0.95 if len(tsc_matches) > 0 else 0.10
    
    return {
        "tsc_mappings": tsc_mappings,
        "tsc_confidence": tsc_confidence,
        "tsc_match_density": min(tsc_match_density, 1.0)
    }

def calculate_framework_confidence(control: Dict[str, Any]) -> str:
    """
    Calculate weighted framework confidence and determine primary framework.
    
    Formula:
    - weighted_score = primary_confidence * 0.6 + match_density * 0.4
    - If weighted_score < 0.70: framework_category = "AMBIGUOUS"
    - Else: framework_category = framework with higher weighted score
    
    Args:
        control: Control dict with both TSC and financial assertion mappings
        
    Returns:
        Framework category: "SOC1", "SOC2", "AMBIGUOUS"
    """
    # TSC scoring (SOC 2)
    tsc_result = map_tsc_criteria(control)
    tsc_confidence = tsc_result['tsc_confidence']
    tsc_density = tsc_result['tsc_match_density']
    tsc_weighted = tsc_confidence * 0.6 + tsc_density * 0.4
    
    # Financial assertion scoring (SOC 1) - already in control
    financial_assertions = control.get('financial_assertions', [])
    if len(financial_assertions) > 0:
        fin_confidence = sum(fa.get('confidence', 0) for fa in financial_assertions) / len(financial_assertions)
        fin_density = len(financial_assertions) / 22  # 22 total assertions
        fin_weighted = fin_confidence * 0.6 + fin_density * 0.4
    else:
        fin_confidence = 0.0
        fin_density = 0.0
        fin_weighted = 0.0
    
    # Store both mappings if confidence > 0.50
    if tsc_confidence > 0.50:
        control['control_tsc_mappings'] = tsc_result['tsc_mappings']
    
    # Determine primary framework
    max_weighted = max(tsc_weighted, fin_weighted)
    
    if max_weighted < 0.70:
        framework = "AMBIGUOUS"
        logging.warning(f"Control {control.get('control_id', 'N/A')}: Low confidence (TSC={tsc_weighted:.2f}, FIN={fin_weighted:.2f}). Manual review needed.")
    elif tsc_weighted > fin_weighted:
        framework = "SOC2"
        logging.info(f"Control {control.get('control_id', 'N/A')}: SOC 2 framework (TSC={tsc_weighted:.2f} > FIN={fin_weighted:.2f})")
    else:
        framework = "SOC1"
        logging.info(f"Control {control.get('control_id', 'N/A')}: SOC 1 framework (FIN={fin_weighted:.2f} > TSC={tsc_weighted:.2f})")
...
def extract_controls_v4(
    ...
    # Step 5.5: Dual framework mapping (Combined reports)
    logging.info("Mapping both TSC and financial assertions to controls...")
    for control in validated_controls:
        # Map financial assertions (SOC 1 methodology)
        financial_assertions = map_financial_assertions(control)
        control["financial_assertions"] = financial_assertions
        
        # Calculate weighted framework confidence
        framework = calculate_framework_confidence(control)
        control["framework_category"] = framework
        
        # Override with PARTIAL_EXTRACTION if incomplete
        if detect_partial_extraction(control) and framework == "SOC1":
    ...
    logging.info(f"Dual framework mapping complete for {len(validated_controls)} controls")

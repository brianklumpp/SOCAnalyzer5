...
GPT_LOG_PATH = config.LOGS_DIR / "cuec_gpt_soc1.log"

# Configure logging to overwrite the log file each time the script runs
CUEC_EXTRACTOR_LOG_PATH = config.LOGS_DIR / "cuec_extractor_soc1.log"
logging.basicConfig(
    ...
    format='%(asctime)s [CUEC_EXTRACTOR_SOC1] %(levelname)s %(message)s',
)
...
CUEC_KEYWORDS = config.CUEC_KEYWORDS_SOC1
...
def extract_cuecs():
    ...
    # Load frameworks dynamically for SOC1
    from ..frameworks import get_available_frameworks
    try:
        available_frameworks = get_available_frameworks(report_type="SOC1")
        logging.info(f"Loaded {len(available_frameworks)} frameworks for SOC1 CUEC mapping: {list(available_frameworks.keys())}")
...
def map_cuec_to_frameworks(cuec_desc, available_frameworks):
    ...
    Returns: (best_tsc_id, best_coso_id, confidence_tsc, confidence_coso, mapping_result)
    """

...
try:
    ...
    print(f"[CONTROL_EXTRACTOR_UNIFIED] Import error: {import_err}")
    raise

# Configure logging
log_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'data', 'logs', 'control_extractor_unified.log')
logging.basicConfig(
    filename=log_path,
    filemode='w',
    level=logging.INFO,
    format='%(asctime)s [CONTROL_EXTRACTOR_UNIFIED] %(message)s',
)
...
def extract_control_with_cot(chunk: Dict[str, Any]) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        # Call GPT
        response = gpt_extract(prompt, "control_extractor_unified")
...
def extract_controls(
    ...
    for chunk in chunks:
        try:
            ...
            logging.info(f"[CHUNK {chunk['chunk_id']}] GPT response time: {gpt_elapsed:.2f}s")

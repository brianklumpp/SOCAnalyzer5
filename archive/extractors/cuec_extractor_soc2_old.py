...
def extract_cuecs():
    ...
    # Load frameworks dynamically (SOC2 by default for CUEC extractor)
    from ..frameworks import get_available_frameworks
    try:
        available_frameworks = get_available_frameworks(report_type="SOC2")
        logging.info(f"Loaded {len(available_frameworks)} frameworks for CUEC mapping: {list(available_frameworks.keys())}")

...
# ============================================================================
# PDF SNIPPET GENERATION - For PDF Viewer Text Search
# ============================================================================

def generate_pdf_snippet(control: Dict[str, Any]) -> Optional[str]:
    """
    Generate a 150-200 char snippet from control text for PDF fuzzy matching.
    
    Uses adaptive logic:
    - Detects control ID patterns (CC1.1, Control 1.1, Control ID: ABC-123, etc.)
    - Extracts ID + first sentence (up to 200 chars)
    - Falls back to first 150 chars if no ID/sentence found
    
    Args:
        control: Control dictionary with 'control_text' field
        
    Returns:
        Snippet string or None if no text available
    """
    if not config.ENABLE_PDF_SNIPPETS:
        return None
        
    text = control.get('control_text', '') or control.get('text', '')
    if not text:
        logging.warning(f"[PDF_SNIPPET] Empty text for control {control.get('control_id', 'UNKNOWN')}")
        return None
    
    # Clean whitespace
    text = ' '.join(text.split())
    
    # Control ID regex patterns (comprehensive)
    id_patterns = [
        r'(CC\d+\.\d+)',                    # CC1.1, CC2.3
        r'(Control\s+\d+\.\d+)',            # Control 1.1
        r'(\d+\.\d+)',                       # 1.1, 2.3
        r'(Control\s+ID:\s*\w+-\d+)',       # Control ID: ABC-123
        r'(Ref:\s*\d+\.\d+\.\d+)',          # Ref: 1.2.3
    ]
    ...
    for pattern in id_patterns:
        ...
        if match:
            ...
            # Find first sentence after ID (up to period, question mark, or exclamation)
            remainder = text[match.end():].strip()
            sentence_match = re.search(r'^[^.!?]*[.!?]', remainder)
            if sentence_match:
                sentence = sentence_match.group(0)
                snippet = f"{control_id} {sentence}".strip()
                # Limit to 200 chars
                if len(snippet) > 200:
                    snippet = snippet[:197] + '...'
                break
    ...
    if not snippet or not snippet.strip():
        logging.warning(f"[PDF_SNIPPET] Generated empty snippet for control {control.get('control_id', 'UNKNOWN')}")
        return None
...
def map_control_to_frameworks_dynamic(
    ...
    Returns:
        Dict mapping framework names to match lists:
        {
    ...
    Note: This function replaces map_control_to_frameworks_multi() for new code.
          The old function is kept for backward compatibility.
    """
    from .. import config
    ...
    for framework_name, framework_data in available_frameworks.items():
        try:
            ...
            framework_prompt = framework_prompt_template.format(
                control_desc=control_desc,
                tsc_criteria_list=criteria_list_text,  # Key name varies by framework
...
def extract_controls_v4(
    ...
    # Step 7: Generate PDF snippets for PDF viewer search (if enabled)
    if config.ENABLE_PDF_SNIPPETS:
        for control in validated_controls:
            snippet = generate_pdf_snippet(control)
            if snippet:
                control["pdf_snippet"] = snippet
        logging.info(f"Generated PDF snippets for {sum(1 for c in validated_controls if 'pdf_snippet' in c)} controls")
